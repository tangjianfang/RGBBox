...
export function useAudioAnalyzer(enabled: boolean, deviceId = '', shouldAnalyze = true): AudioData {
  ...
  useEffect(() => {
    ...
    const getStream = async (): Promise<MediaStream> => {
      // R90.8: source resolution extracted to tools/desktopAudio.ts (shared
      // with the AI listen feature — same sentinels, same fallbacks).
      const stream = await openMonitorStream(deviceId)
