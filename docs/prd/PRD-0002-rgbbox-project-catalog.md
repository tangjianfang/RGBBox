...
### R121. AI8 凭据记住与自动登录——官网窗口不再每次要账号密码（2026-09-18 用户报告「每次在 Electron 中打开官网获取 token 都要输入账号和密码」+「更新 token 时官网主页没有缓存账号密码，请修复」）

> 根因实证（leveldb + API 双向验证）：①token 本身长效（JWT payload exp=签发+**10 年**，实测缓存 token 调 `/user/info` 仍 code:0）；②`persist:ai8` 分区里 `userStore.auth.token` 已落盘——但**官网前端打开时不做登录态恢复**，永远渲染登录页。官网行为改不了，正解=应用侧记住凭据自动登录：`POST /user/login {account,password}`（R111 已逆向，CORS `*`）。
- **R121.1 main/ai8Credentials.ts（新）**：safeStorage 加密存 `{account,password}`（userData/ai8-credentials.json；`isEncryptionAvailable()` 为 false 时退化 base64+标记，行为不崩）；IPC 三条：`ai8SaveCredentials(account,password)` / `ai8ClearCredentials()` / `ai8AutoLogin()`——autoLogin 全程在 main：读凭据 → `shared/ai8Client` 的 `Ai8Client.login()`（R118.1 共享化的兑现：main 直接 import）→ 返回 `{ok,token,account}` 或 `{ok:false,reason}`；**明文密码不跨 IPC**（save 一次性写入后渲染层再也拿不到明文）。
- **R121.2 openOfficialLogin 改造**：先 `ai8AutoLogin()`——成功直接落 token（+syncTokenToProfiles 照旧），**零窗口零输入秒级**；失败/无凭据 → 现官网窗口流程原样兜底（改密/风控验证码场景）。
- **R121.3 凭据管理 UI**：AI8 Tab 底部 token-actions 增「账号密码」展开行（账号+密码输入+保存/清除）：保存=main 存凭据+autoLogin 验证 → 有效则直接落 token，无效提示不落；清除=删凭据（窗口兜底仍可用）。
- **受影响文件**：新 `main/ai8Credentials.ts`；`main/index.ts`（3 handler）、`shared/ipc.ts`、`preload/index.ts`、`AiLabAi8Tab.tsx`（autoLogin 优先+凭据行）、`i18n`（+5 keys）、tests + verify 脚本。
- **验收点**：①typecheck+全量 `yarn test` 0 失败（+凭据 roundtrip/退化存储/autoLogin 协议用例）；②真机 mock：保存凭据 → 点「官网登录」→ 断言 main 发出 `POST /user/login` 与官网形态全等 + token 落地 + **不弹登录窗口**；登录 401 → 落回窗口路径；③无凭据时官网窗口流程零回归；④真实账号实测一键更新 token。
- **实施证据（2026-09-18）**：①typecheck 0 error；②全量 `yarn test` **93 files / 840 passed / 0 失败**（+`tests/main/ai8Credentials.test.ts` 5 用例：加密 roundtrip 且密文不含明文/safeStorage 不可用明文回落仍可读/clear 与坏文件容错/autoLogin POST {account,password} 与官网形态全等且无 Authorization 头/code≠0→reason:'code:N'、空 token→'no-token'）；③真机 CDP `scripts/verify-r121-ai8.mjs` **14/14 PASS**——本地 http mock 站点经 `RGBBOX_AI8_BASE_URL` 测试缝接管 **main 进程** login 流量：凭据行展开/保存 → main 发出 `POST /user/login {"account":"tester@x.com","password":"s3cret-pw"}` 形态全等 → token 落 localStorage → **无窗口弹出**（targets 1→1）→ 磁盘文件 account trim + **password enc:v1: 加密落盘** →「官网登录」二次触发仍 headless（logins=2）→ 错密码 → 中文失败提示 + 凭据保留 → 清除后文件删除；零页面错误。④环境插曲：验证期间 `node_modules/electron/dist` 二进制丢失，按 [[rgbbox-node-env-quirks]] 用 `ELECTRON_MIRROR=npmmirror` 重装恢复；verify 裸 spawn 的 userData 为 `%APPDATA%/Electron`（真实 dev/dist 实例为 rgbbox 目录），脚本已按此断言。**状态：✅（真实账号实测：AI8 Tab →「账号密码」→ 填入并保存 → 以后点「官网登录」即一键无窗更新 token）**
