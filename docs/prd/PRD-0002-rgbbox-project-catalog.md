...
- **R174.10 状态**：✅
- **R178 AI8 桥工具可见性修复（2026-09-26 用户实测「评估 RepoHub 项目」模型回复无法读取目录）**：根因——AI8 桥把 ReAct 工具说明放在站点 systemPrompt 参数里,而该参数对站点部分模型不生效(模型自述"无法读取目录"=完全不知有工具)。修复(Claude 经典模式):新增纯函数 `buildAi8TurnPrompt(workspace, text)`——每轮**用户消息头部直接注入工作区路径+六工具清单+调用约定**,不再依赖站点 system 参数;TOOL_RESULT 消息不带。单测 +1。typecheck 双绿;vitest 128 文件/1160 用例全过。
...
- **R175.8 模型下载二次失败修复（2026-09-26 用户复测 `config.json fetch failed` 0%）**：根因——主进程 Node fetch(undici)**不走系统代理**(渲染进程走,故 AI8 页模型列表正常而模型下载失败)。修复:①下载改走 **Electron `net.fetch`**(Chromium 网络栈,跟随系统代理;非 Electron 环境回落 Node fetch 保可测);②**真·断点续传**——`.part` 分片保留 + `Range: bytes=N-` 追加写(206 才追加,200 重写),291MB 主模型中断不再从头来;③每源 2 次尝试 + 1.5s 退避;④签名收敛为 `xfetch(url,{signal,headers})`。typecheck 双绿;vitest **128 文件/1161 用例全过**。用户验证:重试下载应能走通(系统代理生效)或至少推进百分比(断点续传生效)。
- **R175.9 416 死循环修复（2026-09-26 用户复测 `config.json HTTP 416`）**：根因——上一轮失败的 0 字节/超长 `.part` 触发 Range 请求,站点回 416;而 416 恢复分支的 `rmSync` **未导入**(ReferenceError 被 catch 吞成重试),part 永不删除 → Range/416 死循环直至超时。修复:①补 `rmSync` 导入;②416 → 删分片原地全量重试(不消耗尝试次数);③完成校验改 content-length 精确对账(去 1024 启发式,不再误伤小文件),size mismatch 也删分片重下。回归测试:非零毒分片 → 416 → 全量重下成功 + Range 恰好发一次;vitest **129 文件/1162 用例全过**。：AI8 桥流式(需会话级 SSE 直连,标注后续);agent 工具卡长输出折叠(flash 轮)。
- **R175.6 状态**：✅（R175.4 全过;实机流式观感待用户以 GLM 档跑一次任务确认）

### R176. C1 加固核查（2026-09-26，`feat/ai-lab-expansion` e130ac2；C2 发版前置）

- **R176.1 依赖归属**：`@earendil-works/pi-coding-agent`（26MB+依赖链,运行时零引用,仅 spike 脚本使用）移入 devDependencies——不入应用包;kokoro-js 保留 production（ttsService 动态 import,声文离线引擎依赖）。
- **R176.2 denylist 补全（Windows 盘根三态）**:`rm -rf C:\`(git-bash 风格)、`Remove-Item -Recurse -Force`、`rd /s /q C:\`——原规则只覆盖 `rm -rf /`;测试同步 8 断言。**已知边界**:路径写法的枚举不可能穷尽,审批层(standard 档写/命令全审批)才是主防线,denylist 是 trust 档的最后一道网。
- **R176.3 打包体积核查**：见 R176.5 发版记录（实测数字）;pi 已排除,kokoro-js+@huggingface/transformers 进包为声文离线能力的必要成本。
- **R176.4 状态**：✅

### R177. 精细化打磨方案（规划条款——v0.3.83 发版后的 B 阶段 flash 轮 + 收尾清单;待排期）

> 来源:R175 交付时的遗留标注 + 本轮发版后已知体验缺口。全部为 UI/文案/测试层,**无内核改动**,适合 flash 会话分批执行;每批独立 R-N 带 ui:snapshot 门禁。

| 批 | 内容 | 验收 |
|---|---|---|
| P-1 视觉目检轮 | 9 view 真机目检(苹果尺复评) + agent/voice 两 Tab 专属走查(空态/加载态/错误态/长文本溢出/暗色对比度) | 视觉问题清单归零或入档 |
| P-2 Agent 工具卡折叠 | 工具结果 >N 行默认折叠(点击展开);diff 预览统一配色;流式光标在 done 后移除 | 长输出不再撑爆 transcript |
| P-3 Agent 流式增强 | 每回合思考/文本分段呈现;token 计数与耗时标注(复用 metricsCollector);AI8 桥流式(会话级 SSE 直连,替代整段) | GLM 档首 token <2s |
| P-4 提示词工程 | KERNEL/REACT 系统提示词迭代(参考 Claude Code 的简洁指令风格):工具选择引导/多轮规划/失败重试策略 | 冒烟任务成功率提升可测 |
| P-5 声文增强 | 句级 Kokoro 流式合成(边合成边播)/WAV 导出进度条/词典导入导出(JSON) | 长文本合成不等待 |
| P-6 测试补齐 | agent 工具组边界用例/voiceTab 交互/keep-alive 回归;覆盖率分层回归 | 分层阈值不回退 |
| P-7 遗留清理 | RC-2 GPU 3D 多层缺陷(R165 遗留)/RC-3 screen-ambient 提示形态/B-2 `_maskZone` 保留策略(需产品决策) | 逐项立项关闭 |
...
- **R180.2 自动化测试 +7 用例**：内核循环端到端 4 例（工具往返含审批 once/拒绝后继续/信任免审/并发拒绝——经 SSE 流式响应驱动全链路）；206 断点续传 1 例（2500 字节 .part → Range 精确续尾 → 字节级对账）；ttsModelStatus 完备性 1 例；416 毒分片恢复 1 例（非零毒分片 → 416 → 全量重下，Range 恰发一次）。
- **R180.3 下载链真网络冒烟**：hf-mirror `config.json` 全 GET 200/44 字节（44B 为**合法真实内容**——`style_text_to_speech_2` 架构标记，非 LFS 指针）；对照实验证实 huggingface.co 官方源在本网络被 DNS 污染直连超时 → R175.8 的 net.fetch 跟随系统代理修复方向正确，mirror 优先排序正确。
- **R180.4 UI 布局自适应**：agent/voice 两 Tab 窄窗响应式（≤980px 侧栏上堆、侧栏滚动上限、hover 反馈）；静息快照零影响。
- **R180.5 验收**：typecheck 双绿；vitest **131 文件 / 1168 用例全过**；build 绿；`ui:snapshot` 9/9 GATE PASS。
- **R180.6 状态**：✅


### R182. 下载 416 后续根因修复 + UI 打磨 + 优化总结/新方案（2026-09-26 用户复测「size mismatch 3497/2726297.6」+「UI 不美观」;方案文档 [`2026-09-26-voice-agent-optimization-summary-and-plan.md`](../reviews/2026-09-26-voice-agent-optimization-summary-and-plan.md)）

- **R182.1 下载双根因修复**:①字节预估浮点(2.6*M=2726297.6)致整数对账永败 → 取整;②hf-mirror 对部分请求 200 返回 **3497B HTML 页** → `?download=true` 强制原始文件 + **首 64 字节 HTML 检测**(命中毁片换源重试);③原子写流程回归 3 例全过。
- **R182.2 UI 打磨**:Agent 空态升级(图标+标题+三步引导列表);声文模型栏逐文件内联进度条+行底色+完成淡化;窄窗响应式(≤980px 上堆)。
- **R182.3 优化总结与新方案文档**:已交付 §1 修复时间线(R168→R175.9 七项根因表)/§2 P-1~P-7 打磨批次(视觉走查/工具卡折叠/下载并发/提示词工程/声文流式/测试补齐/遗留清理,flash 可承接)/§3 已知问题清单。
- **R182.4 验收**:typecheck 双绿;vitest 131 文件/1168 用例全过;build 绿;`ui:snapshot` 9/9 GATE PASS(两 Tab 非静息视图,无需重拍)。
- **R182.5 状态**：✅

### R183. P-1 视觉走查轮（R177 方案首批;2026-09-26 用户指令「执行这个优化」→ docs/reviews/2026-09-26-voice-agent-optimization-summary-and-plan.md §2 P-1）

- **R183.1 走查范围**:9 view 真机目检(苹果尺复评:对齐/间距/字阶/层级/色彩纪律)+ AI 实验室 Agent/声文两 Tab 专属走查——空态/加载态/错误态/长文本溢出/窄窗(≤980px)布局/暗色对比度。**实施**:CDP 真机截图 17 份(9 view + Agent/声文 8 态,含独立 user-data-dir + 伪造会话 JSONL 驱动长文本/错误态),双视觉通道交叉评审;走查记录与缺陷清单见 [`docs/reviews/2026-09-26-p1-visual-walkthrough.md`](../reviews/2026-09-26-p1-visual-walkthrough.md)。
...
### R186. P-4 提示词工程 + AI8 重试轮自适应（R177 方案第四批;2026-09-26「执行这个优化」）

- **R186.1 KERNEL/REACT 提示词迭代**(Claude Code 风格:短祈使句、工作流分级):探索先于修改(未读过的文件不写)/最小变更/工具选择引导(read/glob 查看优先,bash 跑测试)/失败策略(读错误→改因→换路子,禁止原样重试,2 次失败即停并报告)/简洁收尾(短 bullet:改了什么+怎么验证,不复述任务)。REACT 协议段补 JSON 合法性约束(双引号/无尾逗号)。
- **R186.2 AI8 桥重试轮自适应**:callModel AI8 分支解析失败时(空回复,或围栏块含 tool/name 意图但 JSON 不合法),注入协议纠正消息重问,每轮上限 2 次;重试耗尽仍有工具意图 → 以错误收束(错误横幅可见),不再把坏块当最终答案静默完成。纯文本最终回答不受影响。
- **R186.3 验收**:typecheck 双绿;vitest **133 文件/1180 用例全过**(+4:提示词约束断言 6 组/坏块→纠正→工具执行/重试耗尽→error 收束/纯文本终答零重试);`ui:snapshot` 9/9 GATE PASS。**冒烟任务(读→建→改→测)成功率 ≥80% 待用户实机 GLM 档复验**(本会话无真实模型凭据;与 R175.6「实机流式观感待用户确认」同例标注——内核侧行为已由 stubbed 端到端测试锁定)。
- **R186.4 状态**:✅(实机冒烟复验待用户)

### R187. P-5 声文增强（R177 方案第五批;2026-09-26「执行这个优化」）

- **R187.1 句级流式合成**:Kokoro 播放改「逐句合成、边合成边播、播 i 时预合成 i+1」队列(整段等待取消);任意句子可点击跳读(kokoro 档同样开放);停止即掐断(可中断续跑=停后再点任意句继续);合成进度「N/M 句」+内联条。WAV 导出保持整段。
- **R187.2 多音色**:全量 55 音色目录(2026-09-26 mirror API 实测列表,含 zf_/zm_ 中文)静态内置;`TtsEngineStatus.voices` 扫描盘上已有音色;未下载音色选中后行内「下载此音色(约 8MB)」→ 新 IPC `tts:voice-download` 走既有 downloadOneFile(Range/HTML检测/对账语义复用);4 预置外的音色不入 KOKORO_FILES(complete 语义不变)。
...
### R189. B 阶段第二批规划（2026-09-26 用户指令「继续规划下一步任务」,圈定五批;各批执行时立独立 R-N）

> 范围来源:P-1 走查入档项(D4/D6/D1/D2/D9)+ R177 原 P-3 未承接部分(Agent 流式增强)+ 用户新增需求(**「智能体和会话功能要方便快速切换、缓存历史记录会话等,按 Claude Code 智能体的执行」**)+ 发版。**设计方向已拍板:中性化+单强调色**(彩色只留给灯效预览本身)。

| 批 | 主题 | 内容 | 验收 | 量 | R-N |
|---|---|---|---|---|---|
| Q-1 滚动/杂项快修 | P-1 入档 D4/D5/D6 | workspace「调节」/diagnostics「环境」滚动容器底部渐隐统一(mask 模式复用);games 下半屏空态引导;ai-config 表单右半空白收窄 | 走查复核销项 | 0.5 会话 | R190 |
| Q-2 Agent 会话管理增强 | 用户新增(仿 Claude Code) | ①「新建会话」显式动作 ②会话重命名/删除 ③会话列表相对时间+事件数 ④打开 Agent Tab 自动恢复最近会话(缓存历史直达) ⑤运行中会话切换不丢(keep-alive 已备,补切换语义) | 会话全生命周期 ≤2 击可达;重启后回到上次会话 | 0.5-1 会话 | R191 |
| Q-3 发版 v0.3.84 | 收口 Q-1/Q-2 + 上轮 6 批 | 用户三项实机复验(P-4 冒烟成功率/P-5 流式合成观感/tokenizer HTML 污染)→修复发现项→`dist:dir` 冷启动验证→`dist:win` | 打包应用可用;体积核查(R176.3 口径) | 0.5 会话 | R192 |
| Q-4 Agent 流式增强 | R177 原 P-3 | 每回合思考/文本分段呈现;token 计数+回合耗时标注(复用 metricsCollector);AI8 桥会话级 SSE 直连(逆向,风险中高) | GLM 档首 token <2s(实机);AI8 流式冒烟 | 1 会话 | R193 |
| Q-5 设计系统统一轮 | 方向已拍板:中性化+单强调色 | 先出 token 方案一页(中性阶/单一强调色/字号 3-4 档/圆角 2 档/间距阶)→实施:模块卡中性化+图标保辨识、≥3 绿收敛单 token、dashboard 状态卡右缘对齐、顶栏与 H1 去重 | 全视图走查复核;基线**一次**重立 | 1-1.5 会话 | R194 |
...
### R194. Q-4 Agent 流式增强（R189 第四批=R177 原 P-3 未承接部分;2026-09-27 用户指令「可以实施下一步」）

> **前提澄清**(用户问「AI8 整段返回无流式是已知无法解决的问题吗」):不是——站点接口本身就是 SSE 流,`collectAi8Reply` 一直在逐 delta 消费,只是攒齐了才返回;"整段"是实现选择。难点仅在:AI8 工具调用以 ```tool``` 文本块出现,流式需分流(围栏前流出、围栏后扣住)。

- **R194.1 AI8 流式**:`ai8ChatCompletion` 增 `onDelta` 透传原始 chunk;内核 AI8 分支**围栏扣留式流式**——首个 ``` 前的文本随 delta 直出,围栏起扣留(3 字符 holdback 防 opener 跨 chunk 漏出);回复无围栏时流尾冲刷(纯散文全量流出);纠偏/协议重问前把已流出的部分收成完整气泡再开新泡(不串泡)。
- **R194.2 回合元数据**:助手气泡附 meta 行——字数 · 首 token 延迟(ms) · 回合总时长(s)(turn-start 计时,首 delta 计首 token;OpenAI 兼容档同步受益)。
- **R194.3 验收**:vitest **137 文件/1213 用例全过**(+4:onDelta 逐 chunk 透传/围栏扣留断言「流出的恰为围栏前文本+终答全文,零 ``` 漏出」/纠偏轮气泡收束/渲染 meta 行);typecheck 双绿;快照 9/9。**GLM 档首 token <2s 与 AI8 真机流式观感待用户复验**。
- **R194.4 状态**:✅(实机观感待用户)

### R195. AI8 卡死治理（2026-09-27 用户报障「Agent/AI8 输入无回复直接卡死,无任何状态,最终 ai8: network — The operation was aborted due to timeout」）

- **根因**:站点接受连接但一条 delta 不发(排队/限流)→ `collectAi8Reply` 干等 **180s 总超时** → abort;更糟的是超时还会触发**会话重建+再等 180s**(重建对"生成卡住"毫无帮助)→ 用户面对数分钟零反馈。
- **R195.1 首token看门狗**:流上 **30s 无任何 delta** → 主动中止,同会话立即重试一次;再超时 → 快速失败(错误信息明确「模型迟迟未响应」),不再吃满 180s。总超时 180s 保留兜底。
- **R195.2 超时不再重建会话**:provider 重建重试仅针对「会话已死」类错误;timeout/abort 类直接透传快速失败。
- **R195.3 运行心跳**:Agent 输入行运行中显示已耗时秒表(每秒走字),用户不再面对"卡死无状态";配合既有 spinner。
- **R195.4 验收**:provider 测试(30s 看门狗触发→重试一次→二次超时快速失败且无 session 重建;首 delta 到达后看门狗解除);组件测试(运行秒表);全量回归;typecheck;快照 9/9。
- **R195.5 状态**:✅

### R196. 声文中英混读（2026-09-27 用户报障「Kokoro 只支持英文,请增加中文支持,中英文同时支持」）

> **上游确认**:kokoro-js 最新即 1.2.1(2025-05),**无中文支持版本可升**;真·Kokoro 品质中文需自研拼音→misaki 音素桥(研究级、音质无保证),列为后续实验项。本轮交付**句级路由**:中英混读句句分明——英文句 Kokoro 合成,中文句系统引擎 zh-CN,同一顺序队列无缝衔接(点击跳读/进度/停止全沿用)。

- **R196.1 混合队列**:`speakKokoroFrom` 改逐句路由——`detectLang`(句级)判 zh → `SpeechSynthesisUtterance(zh-CN)` 单句 await 播放;en → Kokoro 合成+audio 播放(预取仅对 en 句);语速/词典/高亮/进度/停止语义不变。
- **R196.2 提示**:Kokoro 引擎下文本含中文时面板显示混读说明(中文句走系统引擎)。
- **R196.3 验收**:vitest **137 文件/1218 用例全过**(+3:英文句点分句/小数点版本号不切;混合队列 ttsSynthesize 仅收英文句+speechSynthesis 按序收中文句+进度清零)。**取证副产物**:英文句点 `.` 此前根本不参与分句(R173 只按中文标点)——已修(`.` 后跟空白/行尾才切)。typecheck 双绿;快照 9/9。真机听感待用户。
- **R196.4 状态**:✅(被 R197 取代:中文句不再走系统引擎)

### R197. 真·Kokoro 中文（拼音→音素桥;2026-09-27 用户指令「我需要真正支持中文的模型…要导出音频文件」）

> **取证链**:①kokoro-js 最新即 1.2.1,无中文版可升;②其 phonemizer WASM **自带 cmn 语音清单但未捆绑 cmn_dict 词典**(实测 `Can't read dictionary file … cmn_dict`)——espeak 路线被数据卡死;③v1.0 tokenizer 词表 115 字符实测 dump,含 ɕ ʈ ʂ ɤ ɥ ŋ ɻ ɨ 及**音调箭头 ↓→↗↘**(espeak IPA 全集);④kokoro-js **`generate_from_ids(ids,{voice})` 公开且不校验音色名**。结论:自建 汉字→带调拼音(pinyin-pro,多音字实测正确)→IPA 表即可直喂。

- **R197.1 音素桥**(shared/zhPhonemes,纯函数):声母/韵母→IPA 表(送气对/ glide y-w/jqx+u→ü/平翘 i→ɨ)、声调→箭头(1 不标,2↗,3↓,4↘,5 轻)、全角标点→词表符号;输出**全部字符实测在词表内**。
- **R197.2 引擎接线**:ttsSynthesize 按 voice 前缀 zf_/zm_ 路由——`engine.tokenizer(音素串)` → `engine.generate_from_ids(ids, {voice})`(绕过英语 G2P 与音色注册表);导出走 `perSegmentVoices` 逐句分流,**中英混排出单个 WAV**。**真机端到端**:zf_xiaobei 合成「你好，世界。今天是星期天，人工智能正在改变世界。」= **6.95s 音频/3.1s 计算/326KB WAV** ✓。
- **R197.3 UI**:音色目录恢复 8 个中文音色(标「实验」);Kokoro 引擎下新增**中文音色**下拉(独立持久化,默认 zf_xiaobei);混合队列中文句自动换桥音色(系统引擎退出 kokoro 路径);混读提示更新。
- **R197.4 验收**:vitest **138 文件/1223 用例全过**(+5:音节表 10 断言/多音字/标点映射/词表全覆盖×4 句/目录 36+路由一致);typecheck 双绿;快照 9/9。**音质听感待用户试听迭代**(声调映射为首轮约定,箭头语义可按耳朵调)。
- **R197.5 状态**:✅(听感迭代待用户)
...
- **R193.1 会话隔离+记忆最大化**:`ai8Provider` 站点会话改 **per-Agent-会话 Map**(LRU 上限 8);新建站点会话默认参数(与聊天页对齐,弃 `contextCount:0`);**历史回放**——站点会话新建(重启/换会话/崩溃重建)且本地有历史时,首条消息前置压缩回放(纯函数 `buildReplayContext`:逐条截断+总量上限 9K,保首问+近段)。
- **R193.2 工具失明自纠**:模型回复匹配「无法访问/读取文件」类措辞且未出工具块时,注入纠偏重问一次(独立计数,防循环)。
- **R193.3 流残片清理**:未闭合 `<think>`(流被掐)整段剥离,旧正则只匹配闭合对。
