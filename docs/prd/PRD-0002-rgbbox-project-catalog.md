...
- **R116.1 chat 补 model（bug 本体）**：`Ai8ChatOptions` 增 `model?: string`；chat 请求体构造提为导出纯函数 `buildChatBody()`（含 `model` 字段）；`AiLabAi8Tab` 调用处传 `prefs.model`；UI 守卫——模型未选（`prefs.model === ''`）时发送按钮禁用并提示（`ai.ai8.needModel`）。
- **R116.2 消息样式 ChatGPT 化（scope `.ai8` 不影响对话 Tab）**：①正文统一 13px/行高 1.6（`.ai-msg` 此前未设字号继承全局偏大）；②user 消息=右对齐青色 tint 气泡（圆角 12px、max-width 85%、不显示角色标签）；③assistant 消息=全宽无底块+模型名小徽章（查 models label，fallback 'AI'）；④错误消息=红色 tint 提示条 12px（此前 `.ai-msg-error` 完全无样式）；⑤布局：ai8 Tab 时 `.ai-lab` 加 `ai-lab-flush`（overflow hidden），`.ai8` 撑满剩余高、`.ai8-log` 内滚、输入行固定底部（ChatGPT 式，此前整页滚动）；新消息自动滚动到底；清理重复 CSS 声明（`.ai8-input-row .ai8-btn` 双定义等）。
- **R116.3 AI 智能会话标题**：触发=会话第一轮 assistant 回复完成（非 error、内容非空）且未命名过（`Ai8Session.titled?` 标记，先置位防重）；实现=**用当前所选模型**创建临时服务器会话 → 单条消息发送提炼指令（user/assistant 内容各截 600 字符，要求 ≤16 字、只输出标题本身）→ `cleanGeneratedTitle()` 清洗（首行/去引号/去「标题：」前缀/截 24 字）→ 更新本地 title + `updateSession(sessionId,{name})` 同步 ai8 服务器 → 删除临时会话（best-effort）；任何失败静默退回首条消息截断标题；绘画会话（本地 draft 无服务器会话）同样适用。
...
- **实施证据（2026-09-17）**：①typecheck 0 error；②全量 `yarn test` **90 files / 816 passed / 0 失败**（+4 用例：buildChatBody 携带 model/数字 sessionId 与 files 默认、cleanGeneratedTitle 清洗（引号/前缀/多行/截断/空串）、titled 往返）；③真机 CDP `scripts/verify-r116-ai8.mjs`（**mock 全部 ai8 网络流量，零真实积分**）**19/19 PASS**：flush 布局（.ai-lab-flush overflow hidden / .ai8-log 内滚）、模型未选发送禁用+tooltip「请先选择模型」、**/chat/completions body.model=openai_chat::gpt-5.4（bug 修复本体）**、流式回复渲染、**AI 命名端到端**（主会话外独立临时会话发出提炼请求→cleanGeneratedTitle 去引号→侧栏标题「RGB 灯效调优」→updateSession PUT name 同步服务器）、user 青色气泡右对齐/assistant 无底块+模型徽章「Gpt 5.4」/正文 13px、错误红色提示条、长历史自动滚底、全程 0 页面错误；截图 `docs/screenshots/r116-ai8-workbench.png` 入库。④其余四 Tab 样式零回归（改动全部 scope 在 `.ai8`/`.ai-lab-flush` 下，`.ai-msg` 主定义仅合并既有 position 声明，无行为变化）。**环境异常记录（如实入档）**：本轮会话中段发现本机 node 运行时被外部卸载（`D:\Program Files\nodejs` 仅剩 yarn/pnpm shim，node.exe/corepack 全失）——中途用 zip 版 node 24.21.0 临时续跑，用户随后 MSI 重装 node-v24.21.0 恢复；electron 二进制经 npmmirror 镜像重拉（GitHub 直连 ECONNRESET）。**真实站点对话验证（含「模型 是必填项」不再出现）待用户带有效 token 实测一次。状态：✅（真实请求回归挂起用户实测）**
