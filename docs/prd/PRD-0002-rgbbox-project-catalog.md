...
- **阶段证据（2026-09-21，持续追加）**：
  ...
  - **P5 ✅**：`hooks/usePersistedState.ts`（JSON + raw 裸字符串双模式 + '1'/'0' flag 模式，保护既有存储格式零迁移）收敛 App 层 5 处散装 localStorage effect（域 hook 内的保持原样内聚——收敛边际收益低，取舍记录于此）；model3d 死代码已随 P3b 外迁 `Model3DView.tsx`（编译期开关保留 App）；定时器已随 P3b 归属域 hook；**CLAUDE.md 修订**：「Renderer 单 God Component」条款 →「Renderer 是编排层 + 域模块」（View union 不变 / 无路由 store 库 / 帧数据走 ref 三原则），架构树注释同步。终态 App.tsx **702 行**。
