...
## 11. R150 修复批视觉复核(2026-09-22,haiku 会话;4 张全页 + 3 张局部 clip)

> 复核方法同前(temp CDP 内部截图 + 局部 clip);**本节及 §12 为纯 review 产出,未改任何代码**,实施由 opus 会话执行。

| 修复项 | 复核结论 | 证据 |
|---|---|---|
| W1 视频墙截断 | ✅ 落位——「把画布以矩阵方式铺满多块面板,支持拼接补偿与旋转。」完整可读 | `shots-r150/workspace-resolution.png` |
| W2 网格密度值 | ✅ 「320 × 180」单行右对齐 | 同上 |
| W3 fps 警示 pill | ✅ 琥珀容器;小观察:pill 偏宽(fit-content),可接受 | 同上右下角 |
| G2 仪表盘 4×2 | ✅ 两整行无孤儿卡 | `shots-r150/dashboard.png` |
| E1 45→55 | ✅ 副标题「55 种内置效果」 | `shots-r150/effects-classic.png` |
| E2 效果网格 5+2 | ✅ 经典 tab 5 列(5+2),无孤行 | 同上 |
| I1 AI 连接状态 pill | ✅ 「未测试」已入中性灰 pill | `shots-r150/ai-config.png` |
| I3 保存唯一 primary | ✅ 保存为 mint 填充,设为当前 secondary | 同上 |
| A2 音频滑杆 accent | ✅ 音量/平衡滑杆已 mint(局部 clip 确认) | `shots-clips/audio-transport.png` |
| I4 空槽椭圆 | ⏳ 未修(R150.6 留后续),维持原状 | ai-config.png |

**结论:R150 十四项修复全部落位,无回归。**

---

## 12. 设计审查四项(用户点名;供 opus 会话实施,含可执行规格)

### 12.1 仪表盘模块图标——同质圆底 → 每模块专属色相(用户原话:「圆形背景太统一,不方便快速区分」)

**现状**(`DashboardView.tsx:98-101` + `shellModules.ts MODULE_META`):8 张模块卡的圆形图标底全部是同一 mint 实心圆 + 深色剪影(`.dash-tile-icon`),唯一区分是卡底 12px 标签——扫视时无法快速定位目标模块。

**方案:色彩承载识别,几何保持统一**(符合 R148「舞台与观众席」:模块色属内容层 `--fx-*` 家族,不占交互 accent 语义;圆底形状统一 = 优雅):

| 模块 | view | 色相 | hex |
|---|---|---|---|
| 工作区 | `workspace` | mint(品牌锚点) | `#42e8a9` |
| 效果库 | `effects` | violet | `#a78bfa` |
| 视频工作站 | `video` | sky | `#38bdf8` |
| 音频工作站 | `audio` | amber | `#fbbf24` |
| 迷你游戏 | `games` | rose | `#fb7185` |
| 诊断 | `diagnostics` | lime | `#a3e635` |
| 3D 可视化 | `architecture` | orange | `#fb923c` |
| AI 实验室 | `ai` | fuchsia | `#e879f9` |

**实施规格**:
1. `MODULE_META` 加 `tint` 字段(上表 key);
2. `DashboardView` 图标容器渲染 `data-tint={meta.tint}`;
3. CSS 8 变体:`.dash-tile-icon[data-tint='violet'] { background: rgba(167, 139, 250, 0.14); color: #a78bfa; }`(底 12–14% alpha、icon 同色 stroke;暗底低饱和,不抢灯效内容);
4. **rail 不动**——左侧导航是交互控件,active=mint 语义保持;
5. 可选增强:卡片 hover 时图标底 alpha 14%→20% + 卡边框微亮同色。
6. 8 色相在暗底两两可辨(violet/fuchsia/rose 分属蓝紫/粉紫/红粉);未来新增模块按色环顺延并在 `shellModules.ts` 注释登记。

**验收**:1440×900 仪表盘 8 卡一眼可按颜色定位;不引入新渐变/描边;rail 与卡片视觉不冲突。

### 12.2 体感能力清单——单列长表过载 → 分组折叠 + 列瘦身

**现状**(`AiLabVisionTab`):24 能力单列 4 列长表(能力/触发方式/实时/消费处),首屏仅见 ~10 行,分组行(方向 / 捏合 / 手指和弦·命令)淹没在行流;头部 5 按钮一排 + 「未开启」裸文本;右侧快调侧栏恒驻。

**方案(A 分组折叠 + B 列瘦身,改动小、密度骤降)**:
1. **手风琴分组**:表格按现有分组行改为 `<details>` 组——「方向与光标」「捏合与拇指」「手掌状态」「手指和弦」「双手协作」等,组头显示「组名 + n 项 · k 项激活」摘要 + 展开箭头;默认全部折叠,体检运行中自动展开激活组并滚动到位。首屏从 24 行 → 5 组头。
2. **「实时」列撤表入行**:该列只有 `—/活跃` 两态,改为能力名左侧 3px 状态条(灰=未激活/mint 脉动=激活),省一整列,表变三列。
3. **「消费处」瘦身**:单元格只留目标名(`Survival / Tetris / …`),`(R135)` 类编号移入 `title` tooltip 或行下 caption,视觉噪音立减。
4. **头部渐进披露**:未体检时只显示「开始体检 + 重新校准」;体检中才展开「停止/跳过校准/计数清零」;「未开启」接入 `.status-pill`(I1 家族收尾)。
5. 右侧快调侧栏保持;「捏合距离」双柄滑杆加轨道底色与柄径(现过细)。

**验收**:首屏无需滚动即可见全部组头与快调侧栏;体检时激活组自动展开;表格列 4→3。

### 12.3 诊断页——左表右空 → 三组卡 + 右列扩充

**现状**(`DiagnosticsView`):左列 13 行遥测长表一行流,右列仅「各进程 CPU 占用」一张卡(6 行),面板下 60% 空白,PiP 悬右下。

**方案**:
1. **遥测三组卡化**(左列改 2 列网格):
   - 「渲染管线」:虚拟画布 / 网格大小 / 活跃图层 / 目标帧率 / 亮度增益;
   - 「帧与时延」:帧延迟 / 平均帧时 / P95 / Worker 渲染 / 捕获耗时 / 输出耗时 / 丢帧 tick;
   - 「环境」:平台 / 捕获 provider(`getCaptureProviderStatus` IPC 已有:active/fallback/lastCaptureMs——新增展示,功能增强需 R-N)。
2. **值仪表化**:ms/% 类值右对齐 + `.tnum-mono`,加 8px 高 mini-bar(相对阈值刻度),异常段入 `--status-warn`;帧延迟空闲态文案保留现有容器。
3. **右列扩充**:CPU 面板下方补「overlay 帧到达」卡(R48.1 perfSelfTest 通道已有数据结构,日常显示最近一次快照;增强需 R-N);空闲时右列显示引擎空闲说明卡(复用帧延迟空态文案),不再空白。
4. PiP 避让随 G1 全局方案处理。

**验收**:两列网格填满首屏;三组卡各带图标分组头;新数据源展示在 R-N 立项后实施。

### 12.4 音频播放器——进度条通栏 → 三簇分组 transport

**现状**(`AudioStudioView` transport,clip 实测):进度条从 shuffle 右侧直通时间标签,**长 ~1010px 通栏**;时间「0:00 / -:-:-」孤悬右上;第二行才是 暂无播放 + 音量/平衡滑杆——两行共三根长滑杆并排,进度条端点与时间/控制簇距离过远,拖拽PrecISION差。

**方案(推荐布局,纯排版零逻辑改动)**:
1. **单行三簇**:左「播放控制簇」(prev/play/next/loop/shuffle,固定宽)+ 中「进度簇」(当前时间 — 进度条 — 总时长,贴条两端,进度条 `max-width: 520px; flex: 1 1 auto; margin: 0 auto`)+ 右「音量簇」(音量 icon+滑杆 80px + 80% / 平衡滑杆 80px / 保存)。进度条长度 1010→≤520px,精度提升一倍。
2. **时间贴条**:左端当前时间、右端总时长(mono/tabular),删除右上角孤立「0:00 / -:-:-」;时长未知时右端显示 `--:--`。
3. **暂无播放提示**:移到进度条正下居中(caption 级),或作为进度条空态占位——不再独占第二行左端。
4. 可选进阶:声道平衡收进音量按钮 popover(进一步简化右簇);hover 进度条显示预览时间 tooltip。
5. 视频工作站 transport 同构问题较轻(已有 1x/54% 中簇),可后续对齐本规格。

**验收**:进度条 ≤560px 且水平居中;时间紧贴条两端;全行单簇内元素间距 ≤12px;拖拽可用性不低于现状。

### 12.5 实施顺序建议(opus)

1. §12.1 仪表盘图标色相(用户点名,改动小收益大,顺手 R-N 立项);
2. §12.4 音频 transport 三簇(纯 CSS/JSX 排版);
3. §12.2 体感分组折叠(结构改动中等);
4. §12.3 诊断三组卡(其中 2 个新数据源展示需独立 R-N)。

每项独立小批提交;完成后由 haiku 会话再做一轮视觉复核(同 §11 方法)。
